...
class NewsDAOTests {
    ...
    @Test
    fun insert_a_article_in_database () = runBlocking {
    ...
    @Test
    fun delete_all_articles_from_database() = runBlocking {
        val source = Source(null, "24/7 Wall St.")
        val article = Article(
            1,
            source,
        ...
        val before = database.getNewsDAO().getAllArticles().first()

        database.getNewsDAO().deleteAllArticles()
